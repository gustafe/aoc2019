...
my %edges = (rows=>{outer=>{}, inner=>{}}, cols=>{outer=>{},inner=>{}});
...
#                    printf( "step %2d: trying [%2d,%2d]\n", $step, @$try )                      if $debug;
                    ...
                    if ( exists $entries->{$t_r}->{$t_c} ) {
                        my ( $j_r, $j_c ) = @{ $entries->{$t_r}->{$t_c}->{exit} };
			# how do we move between levels?
			my $new_l;
			if ($edges{rows}->{outer}->{$t_r} or $edges{cols}->{outer}->{$t_c} ) {
			    next if $t_l==0; 
			    $new_l = $t_l - 1;

			} elsif ( $edges{rows}->{inner}->{$t_r} or $edges{cols}->{inner}->{$t_c}) {
			    $new_l = $t_l+1;
			} else {
			    die "can't figure out the level: $t_r,$t_c";
			}
                        printf("step %2d: hit %s at [%2d,%2d,%2d], jumping to [%2d,%2d,%2d], adding [%2d,%2d,%2d] to seen list, setting level from %d to %d\n", $step,			       $entries->{$t_r}->{$t_c}->{label},			       $t_r, $t_c,$t_l,			       $j_r, $j_c,	$new_l,		       @{ $entries->{$t_r}->{$t_c}->{entry} },$new_l, $t_l, $new_l ) if $debug;

                        $seen->{ $entries->{$t_r}->{$t_c}->{entry}->[0] }
                          ->{ $entries->{$t_r}->{$t_c}->{entry}->[1] }->{$new_l}++;
                        $try = [ $j_r, $j_c,$new_l];
                    }

                    if ( $t_r == $end->[0] and $t_c == $end->[1] and $t_l==0) {
                        $shortest = $step;
                        last LOOP;
                    }
